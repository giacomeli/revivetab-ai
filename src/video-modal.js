// video-modal.js — player de YouTube embutido em modal, na própria new tab.
...
let _refererRulePromise = null;

function ensureYtRefererRule() {
  ...
  _refererRulePromise = chrome.declarativeNetRequest.updateSessionRules({
    ...
    console.warn('[BD] regra DNR de Referer falhou:', err && err.message);
...
// (Escape/backdrop, via modal.js) remove o iframe do DOM e para a reprodução.
// O link "Abrir no YouTube" cobre vídeos com embed desabilitado pelo dono e
// quem preferir a página completa.
export async function openVideoModal(videoId, originalUrl, title) {
  ...
  showModal(`
    ...
    <div class="flex items-center justify-between gap-3 px-4 py-3 bg-base-200">
      ...
      <a class="btn btn-sm btn-ghost gap-1.5 shrink-0" href="${esc(originalUrl)}">
        ${iconSVG('external-link', 14)}<span>Abrir no YouTube</span>
      </a>
    </div>
